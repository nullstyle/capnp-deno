...
| `connection initialization timed out after Nms`                                            | Service       | A generated service factory or runtime setup exceeded `connectionInitTimeoutMs`                 | Fix slow initialization, raise `connectionInitTimeoutMs`, or move expensive setup outside per-connection factories; see [Service Shutdown](#service-shutdown) |
...
#### `connection initialization timed out after Nms`

**What it means:** A connection was accepted, but the generated service factory
or runtime setup did not finish before `connectionInitTimeoutMs`. The accepted
transport was closed and the attempt was counted in
`handle.stats.failedConnections`.

**How to fix:**

- Keep per-connection factories small and move expensive shared setup to process
  startup.
- Increase `connectionInitTimeoutMs` if normal runtime creation can exceed the
  current window under load.
- Watch `rpc.service.connection_init_timeout` observability events to identify
  the affected service and peer.
- If a timed-out runtime finishes later, capnp-deno closes it as late cleanup;
  still investigate the slow initializer because callers already lost that
  connection.
