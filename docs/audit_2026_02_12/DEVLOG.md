...
- 2026-02-12T10:06 local: Implemented `F-002` fix in `src/rpc/server/bridge.ts`:
  duplicate live `questionId` now returns `Return.exception` with duplicate-id
  reason before answer-table insertion. Added regression test
  `RpcServerBridge rejects duplicate live question ids` in
...
- 2026-02-12T10:06 local: Implemented `F-003` fix in
  ...
  regression test
  `connectWithReconnect default sleep aborts immediately for
  synchronous onRetry abort`
  in `tests/transports/reconnect_edge_test.ts`.
...
- 2026-02-12T10:06 local: Targeted `P2` validation passed:
  ...
  (`28 passed`, `0 failed`).
- 2026-02-12T10:06 local: Implemented `F-005` fix in `tools/capnpc-deno/cli.ts`:
  parser now treats all tokens after `--` as positional schemas. Added
  regression test
...
- 2026-02-12T11:18 local: Closed follow-up AnyPointer decode contract gap in
  `src/rpc/wire/decode.ts` by normalizing runtime AnyPointer decode failures to
  `ProtocolError` for both call params content and return results content.
- 2026-02-12T11:24 local: Follow-up verification passed:
  `deno test tests/wire/rpc_wire_fuzz_test.ts` (`9 passed`, `0 failed`) and
  `just ci-fast` (`875 passed`, `0 failed`).
- 2026-02-12T11:39 local: Identified remaining CI gap: `just ci-real` failed
  because `deno task smoke:real` referenced missing
  `examples/smoke_real_wasm/smoke_real_wasm.ts`.
- 2026-02-12T11:42 local: Added missing smoke example files:
  `examples/smoke_real_wasm/smoke_real_wasm.ts` and
  `examples/smoke_real_wasm/smoke_real_wasm.capnp`.
- 2026-02-12T11:44 local: Real verification gate now passes: `just ci-real`
  succeeded (`smoke:real`, `test:integration`, `test:real` all green).
- 2026-02-12T11:51 local: Hardened `scripts/generate_rpc_schema_ts.sh` for
  cross-platform use by replacing macOS-only `sed -i ''` behavior with OS-aware
  in-place edit handling.
- 2026-02-12T11:52 local: Re-ran `./scripts/generate_rpc_schema_ts.sh`;
...
- 2026-02-12T12:14 local: Full post-migration verification passed:
  `just ci-fast`, `just ci-integration`, and `just ci-real` all succeeded.
