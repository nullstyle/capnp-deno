...
export async function loadCliFileConfig(options: CliOptions): Promise<CliFileConfig | null> {
...
export async function discoverSchemaFiles(srcDirs: string[]): Promise<string[]> {
...
export function renderBarrelModule(paths: string[]): string {
  const entries = [...new Set(paths.map((path) => normalizePath(path)).filter((path) =>
    path !== "mod.ts"
...
function resolveCliFileConfigPaths(config: CliFileConfig, configPath: string): CliFileConfig {
  const baseDir = dirnamePath(normalizePath(configPath));
  const resolve = (value: string): string => resolvePathFromBase(baseDir, value);
  return {
...
function deriveSchemaRelativePath(sourceFilename: string, srcDirs: string[]): string | null {
  const source = normalizePath(sourceFilename);
  const sourceIsAbsolute = isAbsolutePath(source);
  const normalizedRoots = srcDirs.map((root) => trimTrailingSlash(normalizePath(root)))
